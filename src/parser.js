export function parseUpdate(self, str) {
    ...
    if (str[0].substring(0, 1) === 'g') {
        switch (str[0].substring(1, 2)) {
            ...
            case 'z': self.data.zoomPosition = parseInt(str[0].substring(2, 5), 16) - 0x555; break
            case 'f': self.data.focusPosition = parseInt(str[0].substring(2, 5), 16) - 0x555; break
            case 'i':
    ...
    if (str[0].substring(0, 2) === 'ax') {
        switch (str[0].substring(2, 3)) {
            ...
            case 'f': self.data.focusPosition = parseInt(str[0].substring(3), 16) - 0x555; break
            case 'i': self.data.irisPosition = parseInt(str[0].substring(3), 16) - 0x555; break
        }
    }
    ...
    switch (str[0]) {
        case 'dA0': self.data.tally = '0'; break
        case 'dA1': self.data.tally = '1'; break
        case 'p0': self.data.power = '0'; break
        case 'p1': self.data.power = '1'; break
        case 'iNS0': self.data.installMode = '0'; break
        case 'iNS1': self.data.installMode = '1'; break
        case 'd10': self.data.focusMode = '0'; break
        case 'd11': self.data.focusMode = '1'; break
        case 'd30': self.data.irisMode = '0'; break
        ...
        case 'OID': self.data.modelAuto = str[1]
            ...
            break
        case 'OLR':
        case 'TLR': self.data.tally = str[1]; break
        case 'OLG':
        case 'TLG': self.data.tally2 = str[1]; break
        case 'OLY':
        case 'TLY': self.data.tally3 = str[1]; break
        case 'OAF': self.data.focusMode = str[1]; break
        case 'OAW': self.data.whiteBalance = str[1]; break
        case 'OIF':
            self.data.irisLabel =
                str[1] === 'FF'
                    ? 'CLOSE'
            ...
            break
        case 'OIS': self.data.ois = str[1]; break
        case 'OSD':
            switch (str[1]) {
                case 'B1': self.data.colorTemperature = str[2].replace('0x', ''); break
                case '4F': self.data.irisFollowPosition = parseInt(str[2], 16); break
            }
            break
        case 'OSI':
            switch (str[1]) {
                case '18':
                    ...
                    break
                case '20': self.data.colorTempLabel = parseInt(str[2].substring(0, 5), 16).toString() + 'K'; break // VAR
        ...
        case 'OSG':
            switch (str[1]) {
                case '39': self.data.redGainValue = parseInt(str[2], 16) - 0x800; break
                case '3A': self.data.blueGainValue = parseInt(str[2], 16) - 0x800; break
                case '4A': self.data.masterPedValue = parseInt(str[2], 16) - 0x80; break
                case '4C': self.data.redPedValue = parseInt(str[2], 16) - 0x800; break
                //case '4D': self.data.greenPedValue = parseInt(str[2], 16) - 0x800; break
                case '4E': self.data.bluePedValue = parseInt(str[2], 16) - 0x800; break
                //case '5D': self.data.shutterStepLabel = str[2].replace('0x', ''); break // UB300 special case
            }
            break
        case 'OSJ':
            switch (str[1]) {
                case '03': self.data.shutter = str[2].replace('0x', ''); break
                case '06': self.data.shutterStepLabel = '1/' + parseInt(str[2], 16).toString(); break
                case '0F': self.data.masterPedValue = parseInt(str[2], 16) - 0x800; break
                //case '10': self.data.greenPedValue = parseInt(str[2], 16) - 0x96; break
                case '29': self.data.presetSpeedUnit = str[2]; break
                //case '3C': break; // Preset Name / Preset Thumbnail Counter
                case '4A': self.data.colorTempLabel = parseInt(str[2], 16).toString() + 'K'; break // AWB A/B
                ...
                case 'D2': self.data.filter = str[2]; break
            }
            break
        case 'OSL':
            switch (str[1]) {
                case 'B6': self.data.autotracking = str[2]; break // Auto Tracking Mode
                case 'B7': self.data.autotrackingAngle = str[2]; break // Angle
                case 'BB':
                    switch (str[2]) {
                        case '0': self.data.autotrackingStatusLabel = 'Not Tracking'; break
                        case '1': self.data.autotrackingStatusLabel = 'Tracking'; break
            ...
            break
        case 'OGS':
        case 'OGU': self.data.gain = str[1].replace('0x', '').padStart(2, '0'); break
        case 'ORS': self.data.irisMode = str[1]; break
        case 'OTD': self.data.masterPedValue = parseInt(str[1], 16) - 0x1e; break
        case 'OTP': self.data.masterPedValue = parseInt(str[1], 16) - 0x96; break
        case 'ORG': self.data.redGainValue = parseInt(str[1], 16) - 0x1e; break
        case 'OBG': self.data.blueGainValue = parseInt(str[1], 16) - 0x1e; break
        case 'ORI': self.data.redGainValue = parseInt(str[1], 16) - 0x96; break
        case 'OBI': self.data.blueGainValue = parseInt(str[1], 16) - 0x96; break
        case 'ORP': self.data.redPedValue = parseInt(str[1], 16) - 0x96; break
        case 'OBP': self.data.bluePedValue = parseInt(str[1], 16) - 0x96; break
        case 'TITLE': self.data.title = str[1]; break
    }
}
